...
app.post('/retell/function/submit_order', async (req, res) => {
  ...
  if (!cart.items.length) return send(res, 'Cart is empty. Nothing to submit.');
  ...
  // FIXED: Supermenu needs yyyy/MM/dd HH:mm:ss format
  const now = new Date();
  ...
  const m = String(eastern.getMinutes()).padStart(2,'0');
  ...
  console.log('POS endpoint:', CONFIG.pos.endpoint);
  console.log('POS ID:', CONFIG.pos.id);
  console.log('POS password:', CONFIG.pos.pw ? '****' + CONFIG.pos.pw.slice(-3) : 'MISSING');
  ...
  try {
    const r = await axios.post(CONFIG.pos.endpoint, xml, {
      params: { id: CONFIG.pos.id, password: CONFIG.pos.pw },
      headers: { 'Content-Type': 'text/xml' },
      timeout: 15000
    });

    console.log('=== SUPERMENU RESPONSE ===');
    console.log('Status:', r.status);
    console.log('Body:', typeof r.data === 'string' ? r.data : JSON.stringify(r.data));

    delete carts[cid];
    const eta = ot==='delivery'?'30 to 45 minutes':'15 to 20 minutes';
    send(res, `Order placed! Reference #${ref}. Total $${tot}. Ready in about ${eta}.`);
  } catch(e) {
    console.error('=== SUPERMENU ERROR ===');
